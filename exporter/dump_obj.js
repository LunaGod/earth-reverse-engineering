...
const [DUMP_OBJ_DIR, DUMP_JSON_DIR, DUMP_RAW_DIR] = ["obj", "json", "raw"].map(
  (x) => path.join(DL_DIR, x)
);
const { OCTANTS, MAX_LEVEL, DUMP_JSON, DUMP_RAW, PARALLEL_SEARCH } =
  require("./lib/parse-command-line")(__filename);
...
function writeNodeOBJ(ctx, node, nodeName, exclude) {
  for (const [meshIndex, mesh] of Object.entries(node.meshes)) {
    ...
    fs.appendFileSync(
      path.join(ctx.objDir, "model.mtl"),
      `
			newmtl ${texName}
...
function writeMeshOBJ(ctx, meshName, texName, payload, mesh, exclude) {
  function shouldExclude(w) {
    return exclude instanceof Array ? exclude.indexOf(w) >= 0 : false;
  }
  ...
  if (mesh.uvOffsetAndScale) {
    ...
    for (let i = 0; i < vertices.length; i += 8) {
      ...
      const tex = mesh.texture;
      if (tex.textureFormat == 6) console.log(`vt ${ut} ${1 - vt}`);
      else console.log(`vt ${ut} ${vt}`);
      c_u++;
    }
  }
  ...
  for (let i = 0; i < indices.length - 2; i += 1) {
    ...
    triangle_groups[w] = [].concat(triangle_groups[w] || [], [
      i & 1 ? [a, c, b] : [a, b, c],
    ]);
  }
  ...
  for (let k in triangle_groups) {
    ...
    for (let t in triangles) {
      ...
      if (mesh.uvOffsetAndScale) {
        console.log(
          `f ${a + _c_v}/${a + _c_u}/${a + _c_n} ${b + _c_v}/${b + _c_u}/${
            b + _c_n
          } ${c + _c_v}/${c + _c_u}/${c + _c_n}`
        );
      } else {
